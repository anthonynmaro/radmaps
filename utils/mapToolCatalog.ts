...
export const MAP_TOOL_CATALOG: MapToolCatalogItem[] = [
  ...
  },
  {
    ...
    notes: 'Staging now has validated contiguous-US, North America, New Zealand, Northern Spain/Camino, and Mount Fuji/Japan Planetiler PMTiles base atlases in R2. tiles.radmaps.studio is live through the Cloudflare Worker custom domain, with the Vercel shim retained as fallback during DNS cache transition. The next coverage queue is tracked in atlas/coverage-targets.json: promote North America after QA, review the live global proof packs, then build Patagonia before any larger Europe/global work. The wider Honshu/Japan attempt needs a larger runner or smaller regional splits after exit 137 on ubuntu-latest. Production still points at Driftless until Atlas Lab, editor, Browserless, attribution, analytics, and rollback QA pass. This is the strategic replacement for most commercial vector/raster basemap dependency.',
  },
  {
