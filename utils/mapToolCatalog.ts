...
export const MAP_TOOL_CATALOG: MapToolCatalogItem[] = [
  {
    ...
    runtimeSurfaces: ['Editor MapPreview', 'AWS renderer proof/final renders', 'Theme previews'],
  ...
  {
    id: 'mapbox',
    name: 'Mapbox Maps, Streets, Fonts',
    provider: 'Mapbox + OpenStreetMap + terrain suppliers',
    status: 'active',
    costModel: 'usage-based',
    appNames: ['topographic', 'Mapbox Outdoors', 'Mapbox Streets overlay'],
    usedByPresets: ['topographic', 'route-only', 'road-network', 'contour-art', 'native-toner', 'optional overlays across raster presets'],
    runtimeSurfaces: ['Editor MapPreview', 'AWS renderer proof/final renders', 'Glyph loading'],
    sources: ['Mapbox Outdoors raster style tiles', 'mapbox-streets-v8 vector tiles', 'Mapbox font glyphs'],
    capabilities: ['Editable vector roads', 'Editable vector water', 'Place labels', 'POI labels', 'Glyphs for contour labels'],
    layerAttributes: ['show_roads', 'roads_color', 'roads_opacity', 'water_color', 'show_place_labels', 'place_labels_color', 'place_labels_scale', 'show_poi_labels'],
    attribution: 'Requires Mapbox and OpenStreetMap attribution; satellite tiles may add Maxar or other imagery credits.',
    spendRisk: 'High leverage dependency because it backs visible styles, glyphs, and optional vector overlays.',
    trackingKeys: ['provider_id', 'preset', 'uses_roads', 'uses_labels', 'uses_pois', 'uses_contours', 'uses_hillshade', 'render_class'],
    notes: 'Best current source for editable vector overlays, but not a product contour fallback; runtime contours come from maplibre-contour.',
  },
  {
    id: 'maptiler',
    ...
    runtimeSurfaces: ['Editor MapPreview', 'AWS renderer proof/final renders'],
  ...
  },
  {
    ...
    capabilities: ['Illustrative watercolor raster', 'High-contrast toner raster', 'Legacy Toner label/no-label tile family switch'],
    ...
    notes: 'Kept as the legacy provider-backed Watercolor/Toner option for comparison and existing saved maps. Toner Light and Toner Dark are now the first-party visual recreation path and should not call Stadia raster tiles.',
  },
  {
    id: 'aws-mapzen-dem',
    ...
    appNames: ['mapbox-dem source name', 'AWS/browser contour DEM', 'Hillshade DEM'],
  ...
  },
  {
    ...
    appNames: ['radmaps-vector', 'radmaps-roads', 'radmaps-water', 'radmaps-labels', 'radmaps-toner-light', 'radmaps-toner-dark', 'Toner Light', 'Toner Dark', 'RadMaps Field Topo', 'RadMaps Contour Wash', 'RadMaps Watercolor', 'RadMaps Night Relief', 'RadMaps Simple Contour'],
    usedByPresets: ['radmaps-toner-light', 'radmaps-toner-dark', 'Atlas Lab house styles', 'Future default for route-only', 'road-network', 'contour-art', 'native-toner', 'new house styles'],
    runtimeSurfaces: ['Hosted tiles.radmaps.studio edge', 'Admin Atlas Lab live PMTiles previews through /api/atlas/tiles', 'Editor MapPreview', 'AWS renderer proof/final renders', 'Analytics attribution ledger'],
    sources: ['Cloudflare R2 PMTiles generated by Planetiler, Protomaps, or Tilemaker', 'Hosted /tiles/{environment}/{artifactId}/{z}/{x}/{y}.mvt contract backed by PMTiles range reads', 'Same-origin MVT tile endpoint fallback backed by the same R2 manifests', 'Overture Places bbox-filtered PMTiles overlays under the existing poi artifact key', 'OSM route=hiking, route=bicycle, and route=mtb relation overlays under outdoorRoutes'],
    capabilities: ['Separate water polygons', 'Waterways', 'Road/transportation polygons with line fallback styling', 'Trails/paths from transportation', 'Named outdoor-route overlays', 'Buildings', 'Landuse', 'Parks/forests', 'Place labels', 'Base POI classes plus additive Overture POI overlays', 'First-party light/dark Toner variants with high-contrast road linework and soft dot patterns for named protected park areas', 'Noto Sans glyphs for Atlas Lab labels'],
    layerAttributes: ['Full vector paint/layout control for roads, water, labels, POIs, landcover, buildings, trail paths, and outdoor route overlays', 'transportation geometry type must be documented per atlas version before style promotion', 'poi overlays use the existing poi manifest key; named route overlays use outdoorRoutes/outdoorRoute'],
    ...
    notes: 'Production now has validated Driftless, contiguous-US, North America, New Zealand, Northern Spain/Camino, Mount Fuji/Japan, Patagonia Andes, Western Alps/Dolomites, Atlantic islands, Peru/Ecuador Andes, Nepal Himalaya, Iceland, Scotland, and Costa Rica Planetiler PMTiles base atlases in R2. The next coverage queue is tracked in atlas/coverage-targets.json v2 with a $200 total build budget gate, z16 POI/outdoor-route hotspot overlays, dry-run cost logging, concrete print-QA fixture bboxes, and 24x36 AWS-rendered print QA before broad marketing. tiles.radmaps.studio is live through the Cloudflare Worker custom domain; the same-origin app proxy now returns empty overlay MVTs for sparse poi/outdoorRoutes coverage and has HEAD probes for monitoring. Toner Light and Toner Dark use these vector layers plus generated MapLibre dot patterns on named protected park/preserve polygons to replace the Stadia visual dependency for first-party Toner without blanket texture. This is the strategic replacement for most commercial vector/raster basemap dependency.',
  },
  {
    ...
    runtimeSurfaces: ['Server PNG watercolor tile endpoint', 'Editor MapPreview', 'AWS renderer proof/final renders'],
    ...
    spendRisk: 'Moves watercolor rendering cost into first-party server CPU, tile cache, and AWS renderer tile waits; no per-tile third-party watercolor API spend.',
  ...
  },
  {
    ...
    notes: 'Runtime contours are the default scale strategy. Staging also has 177 us-terrain-phase1 contour shard artifacts for QA/cache experiments, while older Driftless/Yosemite/Rocky Mountain/Smokies/North Shore packs are retained as fallback/history. Do not build global high-detail contour PMTiles unless AWS renderer reliability or paid demand proves the cache is worth it.',
  },
  {
    id: 'naip-aerial',
    ...
    runtimeSurfaces: ['Editor MapPreview', 'AWS renderer proof/final renders', 'Regional tile builder'],
