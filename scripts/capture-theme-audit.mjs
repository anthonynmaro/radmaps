...
async function collectImageSemanticChecks(entry, printFile, geometry) {
  ...
  if (entry.themeId === 'night-ride') {
    const ink = hexToRgb('#0E1622')
    const cyanRoutePixels = await countPixelsForRegion(printFile, mapRect, (r, g, b) =>
      r > 20 && r < 85 && g > 165 && g < 230 && b > 165 && b < 235,
    )
    groups.palette.push(
      semanticCheck('Night Ride poster dominant color is night navy', colorDistance(fullAverage, ink) < 44, `${formatRgb(fullAverage)} vs #0E1622`),
    )
    groups.routeStyling.push(
...
async function collectSemanticChecks(page, entry, geometry, editorGeometry = null) {
  ...
  if (entry.themeId === 'night-ride') {
    ...
    groups.palette.push(
      semanticCheck('Night Ride dark navy background', String(style.background_color).toUpperCase() === '#0E1622', String(style.background_color ?? '')),
      semanticCheck('Night Ride label background matches night navy', String(style.label_bg_color).toUpperCase() === '#0E1622', String(style.label_bg_color ?? '')),
    ...
    groups.mapLayers.push(
      ...
      semanticCheck('Night Ride dark land token configured', String(atlasLayerSettings.landcover?.color ?? '').toUpperCase() === '#0E1622', JSON.stringify(atlasLayerSettings.landcover ?? {})),
