...
async function collectSemanticChecks(page, entry, geometry, editorGeometry = null) {
  ...
  if (entry.themeId === 'brutalist') {
    ...
    groups.mapLayers.push(
      ...
      semanticCheck('Brutalist contours use visible pale minors and bold black index weight', Number(style.contour_minor_width ?? 0) >= 0.55 && Number(style.contour_minor_width ?? 0) <= 0.70 && Number(style.atlas_layer_settings?.contour?.minor_opacity ?? 0) >= 0.30 && Number(style.atlas_layer_settings?.contour?.minor_opacity ?? 0) <= 0.42 && Number(style.contour_major_width ?? 0) >= 0.95 && Number(style.contour_major_width ?? 0) <= 1.22 && Number(style.atlas_layer_settings?.contour?.major_opacity ?? 0) >= 0.68 && Number(style.atlas_layer_settings?.contour?.major_opacity ?? 0) <= 0.80, `${style.contour_minor_width}/${style.atlas_layer_settings?.contour?.minor_opacity}/${style.contour_major_width}/${style.atlas_layer_settings?.contour?.major_opacity}`),
  ...
  if (entry.themeId === 'night-ride') {
    ...
    groups.mapLayers.push(
      ...
      semanticCheck('Night Ride dark land token configured', String(atlasLayerSettings.landcover?.color ?? '').toUpperCase() === '#101621', JSON.stringify(atlasLayerSettings.landcover ?? {})),
