...
|---|---|---|---|
| `staging` | Contiguous U.S., North America, New Zealand, Northern Spain/Camino, and Mount Fuji/Japan base artifacts in R2. Main artifact ids: `radmaps-us-base`, `radmaps-north-america-base`, `radmaps-new-zealand-outdoor-base`, `radmaps-northern-spain-camino-base`, `radmaps-mount-fuji-japan-base`. | `177` `us-terrain-phase1` contour shards retained for QA/cache experiments; default strategy is browser-rendered contours. | Hosted at `tiles.radmaps.studio` through the Cloudflare Worker custom domain and verified at `radmaps-atlas-tiles.radmaps-atlas.workers.dev`; the Vercel shim remains as fallback during DNS cache transition. |
...
- Coverage expansion targets live in `atlas/coverage-targets.json`. North
  America is the promote-first path; New Zealand, Northern Spain/Camino, and
  Mount Fuji/Japan are live low-cost global proof packs. Patagonia remains the
  next small build candidate. Larger hotspots such as Alps/Dolomites and
  Himalaya stay deferred until source-size, demand, and DEM quality gates
  clear.
