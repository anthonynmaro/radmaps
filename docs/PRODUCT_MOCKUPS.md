...
- Current compositor version: `template-asset-compositor-v21`.
...
Gloss acrylic templates replay circular rivet crops from the saved template
above the inserted map, while keeping the acrylic glare as a separate finish
overlay. The crops are centered on the baked hardware rather than aligned to
the replacement artwork edge, so the screws keep the Gelato material styling
without pulling in the old template corners.
