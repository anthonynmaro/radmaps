...
describe('theme application', () => {
  ...
  it('applies Dark Sky without carrying stale leader-line or segment-casing state', () => {
    ...
    expect(next.atlas_layer_settings?.landcover?.color).toBe('#101A38')
    expect(next.atlas_layer_settings?.landcover?.opacity).toBe(0)
    expect(next.atlas_layer_settings?.park?.fill_color).toBe('#101A38')
